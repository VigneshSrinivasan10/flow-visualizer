"""Training script for Flow Matching model - exact copy from scaling-recipes."""
...
def main(cfg: DictConfig) -> None:
    """Main training function - exact copy from scaling-recipes."""
    ...
    # Create model with width from config
    logger.info("Creating model...")
    model = FlowMLP(width=cfg.model.width, n_blocks=cfg.model.n_blocks)

    # Use the exact optimizer configuration from scaling-recipes
    ...
    for epoch in tqdm(range(cfg.training.n_epochs), desc="Training"):
        ...
        # This is the exact training loop from scaling-recipes
