"""Visualization script for Adjoint Matching model."""
...
def create_trajectory_curvature_animation(
    ...
    def update(frame):
        ...
        ax.set_title("Adjoint Matching: Trajectory Curvature", fontsize=14, fontweight="bold")
        ax.set_xlim(-4.5, 4.5)
        ax.set_ylim(-2.8, 2)
        ax.set_aspect("equal")
...
def create_probability_path_animation(
    ...
    def update(frame):
        ...
        ax.set_title("Adjoint Matching: Probability Path", fontsize=14, fontweight="bold")
        ax.set_xlim(-4.5, 4.5)
        ax.set_ylim(-2.8, 2.2)
        ax.set_aspect("equal")
...
def main(cfg: DictConfig) -> None:
    """Main visualization function for adjoint matching."""
    logger.info("Starting adjoint matching visualization...")
    ...
    logger.info("Loading adjoint matching model...")
    velocity_net = FlowMLP(
        width=cfg.model.width,
        n_blocks=cfg.model.n_blocks,
    )

    model_path = Path(cfg.training.output_dir) / "adjoint_matching_velocity_net.pt"
    ...
    create_trajectory_curvature_animation(
        ...
        output_dir / "adjoint_trajectory_curvature.gif",
    ...
    create_probability_path_animation(
        trajectory,
        target_data[:n_vis_samples],
        output_dir / "adjoint_probability_path.gif",
        fps=cfg.visualization.get("animation_fps", 20),
        dpi=cfg.visualization.get("animation_dpi", 100),
    ...
    logger.info("Adjoint matching visualization complete!")
